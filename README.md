...
<img src="images/Clignotement_LED.jpeg" alt="Clignotement de la LED" width="300"/>
...
<img src="images/test_uart2.png" alt="Test de Printf" width="700"/>
...
<img src="images/test_shell_tache.png" alt="Test Shell" width="700"/>
...
Signal de l'Horloge **MCLK** : 

![Signal de l'Horloge MCLK mesuré avec l'oscilloscope](images/mclk.png)
...
### 1. Dérivation de l'Équation Différentielle

Le circuit est un filtre RC passe-bas passif. La loi des mailles (ou la loi d'Ohm pour R et la loi du condensateur pour C) permet d'établir l'équation différentielle régissant le système.

**Lois Fondamentales :**
* Tension d'entrée : $V_{in}(t) = V_R(t) + V_{out}(t)$
* Courant dans le circuit : $I(t) = C \cdot \frac{dV_{out}(t)}{dt}$
* Loi d'Ohm pour R : $V_R(t) = R \cdot I(t)$

**Substitution :**
$$
V_{in}(t) = R \cdot \left( C \cdot \frac{dV_{out}(t)}{dt} \right) + V_{out}(t)
$$

**Forme Demandée :**
$$
V_{in} = \underbrace{(RC)}_{X} \cdot \frac{dV_{out}(t)}{dt} + \underbrace{1}_{Y} \cdot V_{out}
$$
...
### 2. Équation de Récurrence Discrète (Numérique)

Pour la conversion numérique, nous remplaçons les termes continus par leurs équivalents discrets (méthode de différence finie avant ou *Forward Euler*), en utilisant la substitution demandée :

$$
\frac{dV(t)}{dt} \approx f_s \cdot (V[n] - V[n-1])
$$
Où $f_s = 1/T$ est la fréquence d'échantillonnage et $T$ est la période d'échantillonnage.

**Substitution dans l'équation différentielle :**
$$
V_{in}[n] = RC \cdot f_s \cdot (V_{out}[n] - V_{out}[n-1]) + V_{out}[n]
$$

**Objectif :** Isoler la nouvelle sortie $V_{out}[n]$ pour obtenir l'équation de récurrence.

1.  **Développer le terme RC :**
    $$
    V_{in}[n] = RC \cdot f_s \cdot V_{out}[n] - RC \cdot f_s \cdot V_{out}[n-1] + V_{out}[n]
    $$

2.  **Regrouper les termes $V_{out}[n]$ :**
    $$
    V_{in}[n] = V_{out}[n] \cdot (1 + RC \cdot f_s) - RC \cdot f_s \cdot V_{out}[n-1]
    $$
...
4.  **Résoudre pour $V_{out}[n]$ :**
    $$
    V_{out}[n] = \frac{V_{in}[n] + RC \cdot f_s \cdot V_{out}[n-1]}{1 + RC \cdot f_s}
    $$

**Forme Demandée :**
$$
V_{out}[n] = \frac{\underbrace{1}_{A} \cdot V_{in}[n] + \underbrace{(RC \cdot f_s)}_{B} \cdot V_{out}[n-1]}{\underbrace{(1 + RC \cdot f_s)}_{D}}
$$

---

### 3. Expressions des Coefficients A, B et D

L'énoncé demande de remplacer le produit $RC$ par $\frac{1}{2\pi f_c}$, où $f_c$ est la fréquence de coupure du filtre.

Soit $\tau = RC$ la constante de temps.

**Substitution de $\tau$ :**
$$
RC = \tau = \frac{1}{2\pi f_c}
$$

**Simplification :**
Introduisons la variable sans dimension $\alpha$ :
$$
\alpha = RC \cdot f_s = \frac{f_s}{2\pi f_c}
$$

Les coefficients sont donc :

* **A** : $A = 1$
* **B** : $B = RC \cdot f_s = \frac{f_s}{2\pi f_c} = \alpha$
* **D** : $D = 1 + RC \cdot f_s = 1 + \frac{f_s}{2\pi f_c} = 1 + \alpha$

**Équation de récurrence finale pour le codage :**
$$
V_{out}[n] = \frac{1}{1 + \alpha} \cdot V_{in}[n] + \frac{\alpha}{1 + \alpha} \cdot V_{out}[n-1]
$$
Où $\alpha = \frac{f_s}{2\pi f_c}$.

---

### 4. Calcul de Performance du Processeur

**Question :** Pour une fréquence d'échantillonnage $f_s = 48 \text{ kHz}$, combien de cycles processeurs disposons-nous pour traiter chaque échantillon ?

Ce calcul détermine la charge maximale que le DSP ou microcontrôleur peut gérer avant de manquer le prochain échantillon.

**Données :**
* Fréquence d'échantillonnage ($f_s$) = $48000 \text{ échantillons/seconde}$
* Fréquence du processeur ($f_{clk}$) = $X \text{ Hz}$ (à déterminer ou supposer)

**1. Période de Traitement ($T_{sample}$) :**
C'est le temps maximum disponible pour traiter un seul échantillon.
$$
T_{sample} = \frac{1}{f_s} = \frac{1}{48000} \approx 20.83 \text{ microsecondes}
$$

**2. Nombre de Cycles Processeurs ($N_{cycles}$) :**
Le nombre de cycles disponibles par échantillon est donné par :
$$
N_{cycles} = T_{sample} \cdot f_{clk}
$$

* **Exemple (si le processeur tourne à $100 \text{ MHz}$) :**
    $$
    N_{cycles} = 20.83 \times 10^{-6} \text{ s} \cdot 100 \times 10^6 \text{ Hz} = 2083 \text{ cycles/échantillon}
    $$

* **Exemple (si le processeur tourne à $500 \text{ MHz}$) :**
    $$
    N_{cycles} = 20.83 \times 10^{-6} \text{ s} \cdot 500 \times 10^6 \text{ Hz} \approx 10416 \text{ cycles/échantillon}
    $$

**Réponse Générale :**

Nous disposons d'un temps de **$20.83 \text{ microsecondes}$** (ou $\frac{1}{48000}$ secondes) pour effectuer tous les calculs (lecture ADC, application du filtre, écriture DAC). Le nombre exact de cycles processeurs disponibles dépend de la **fréquence d'horloge ($f_{clk}$) de votre microcontrôleur** selon la formule $N_{cycles} = \frac{f_{clk}}{48000}$.
